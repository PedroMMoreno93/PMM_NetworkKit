...
public final class DefaultNetworkClient: NetworkClient {
    ...
    public func send<E: Endpoint>(
        ...
        for interceptor in interceptors {
            request = try await interceptor.prepare(
                request,
                endpoint: endpoint
            )
        }
        ...
        for interceptor in interceptors {
            processed = await interceptor.didReceive(
                processed,
                for: request,
                endpoint: endpoint
            )
        ...
        case .success(let (http, data)):
            try Self.assertAcceptableStatus(
                http: http,
                data: data
            )
            return try decode(
...
internal extension DefaultNetworkClient {
    func buildRequest<E: Endpoint>(
        for endpoint: E
    ) throws -> URLRequest {
    ...
    static func assertAcceptableStatus(http: HTTPURLResponse, data: Data) throws {
        ...
        case 500...599:
            throw NetworkError.server(status: http.statusCode, payload: data)
        default:
            throw NetworkError.unacceptableStatus(code: http.statusCode, payload: data)
        }
    }
    ...
    func decode<E: Endpoint>(endpoint: E, http: HTTPURLResponse, data: Data) throws -> E.Response {
        ...
        case .raw:
            ...
            if E.Response.self == Data.self, let typed = data as? E.Response {
                return typed
            }
            ...
            throw NetworkError.decoding(
                underlying: DecodingError.typeMismatch(
                    E.Response.self,
                    .init(codingPath: [], debugDescription: "Expected Data response")
                )
            )
        ...
        case .json(let maybeDecoder):
            do {
                ...
                let value = try dynamicJSONDecode(
                    E.Response.self,
                    from: data,
                    using: decoder
                )
